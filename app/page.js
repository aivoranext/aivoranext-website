...
export default function Home() {
  ...
  // Global scroll for navbar hide/show
  const { scrollYProgress: globalScrollProgress } = useScroll();
  
  useMotionValueEvent(globalScrollProgress, "change", (current) => {
    if (typeof current === "number") {
      const direction = current - globalScrollProgress.getPrevious();
      
      // Always show at top of page
      if (globalScrollProgress.get() < 0.05) {
        setNavVisible(true);
      } else {
  ...
  return (
    <main className="relative min-h-screen bg-[#050505] overflow-x-hidden">
      ...
      {/* Navigation - Sticky header with scroll hide/show effect */}
